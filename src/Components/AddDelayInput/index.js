    ...
    const updatedTrainSchedule = trainSchedule.map(train => {
        if(train.trainNumber === trainNumber) {
            ...
            const calculateNewArrivalTime = (time) => {
                ...
                const [delayHours, delayMinutes] = delayTimeString.split(":").map(item => Number(item));
                const totalDelayInMs = (delayHours * 60  + delayMinutes) * 60 * 1000 ;

                const newArrivalTimeInISOString = new Date(totalDelayInMs + getCurrentArrivalTime);
                const now = new Date();
            ...
            newArrivalTimeString = `${tempNewArrivalDateObject.getHours() < 10 ? `0${tempNewArrivalDateObject.getHours()}` : tempNewArrivalDateObject.getHours()}:${tempNewArrivalDateObject.getMinutes()}`

            return {...train, arrivalTime : newArrivalTime, arrivalTimeString : newArrivalTimeString }
        }
